...
export class HomePage {
  ...
  async render(container) {
    ...
    try {
      ...
      const titleEl = container.querySelector("#overview-title");
      if (titleEl)
        titleEl.textContent = `Today's Overview - ${format(today, "EEEE, MMMM do, yyyy")}`;

      // Nutrition summary
      const nutritionSlot = container.querySelector("#nutrition-summary-slot");
      if (nutritionSlot)
        nutritionSlot.innerHTML = this.renderNutritionSummary(todaysNutrition);

      // Quick actions
      const quickActionsSlot = container.querySelector("#quick-actions-slot");
      if (quickActionsSlot)
        quickActionsSlot.innerHTML = this.renderQuickActions();

      // Meals list
      const mealsTitle = container.querySelector("#meals-title");
      if (mealsTitle)
        mealsTitle.textContent = `Today's Meals (${todaysMeals.length})`;
      const mealsSlot = container.querySelector("#meals-list-slot");
      if (mealsSlot) {
        mealsSlot.innerHTML =
      ...
      const mealsSlot = container.querySelector("#meals-list-slot");
      if (mealsSlot) {
        mealsSlot.innerHTML = `<div class="alert alert-error"><strong>Error:</strong> Failed to load today's data. ${error.message}</div>`;
      } else {
        container.innerHTML = `<div class="container"><div class="alert alert-error"><strong>Error:</strong> Failed to load today's data. ${error.message}</div></div>`;
      }
    }
  }
  ...
  openBarcodeScanner() {
    ...
    const barcode = prompt(
      "Enter barcode (or use camera scanning in future version):",
    );
    if (barcode) {
