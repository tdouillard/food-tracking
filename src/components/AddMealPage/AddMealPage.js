...
export class AddMealPage {
  constructor(meal, edit = false) {
    this.meal = meal; // If provided, we're editing an existing meal
    this.edit = edit;
    this.showHistory = !edit; // Show history sidebar only in Add mode
  ...
  async render(container) {
    ...
    this.meal && this.prefillFromMeal(this.meal);
    this.showHistory && this.loadMealHistory();
  ...
  attachEventListeners(container) {
    ...
    // Barcode scanning
    container
      .querySelector("#scan-product-barcode")
  ...
  renderMealHistory(meals) {
    const historyCard = document.getElementById("meal-history-card");
    ...
    list.innerHTML = meals
      ...
      .map((m) => {
        ...
        return `<div class="meal-history-item" data-meal-id="${m.id}">
        <div><strong>${m.name || "Meal"}</strong></div>
        <div class="meal-history-meta">${this.formatShortDate(m.timestamp)}</div>
        <div class="meal-history-kcal">${kcal} kcal</div>
      </div>`;
      })
      .join("");
    list.querySelectorAll(".meal-history-item").forEach((el) => {
      el.addEventListener("click", () => {
        const meal = this.allMeals.find(
          (m) => m.id === el.getAttribute("data-meal-id"),
        );
        if (meal) {
          this.prefillFromMeal(meal);
          list
            .querySelectorAll(".meal-history-item")
  ...
  prefillFromMeal(meal) {
    // Meal name & datetime
    const nameInput = document.getElementById("meal-name");
    const dtInput = document.getElementById("meal-datetime");
    if (nameInput) nameInput.value = meal.name || "";
    if (dtInput && meal.timestamp) {
      dtInput.value = this.toLocalDatetimeInput(meal.timestamp);
      // Store selected date
      this.selectedDate = new Date(dtInput.value);
    }

    // Products
    this.selectedProducts = (meal.products || []).map((p) => ({ ...p }));
    // If historical products missed calculatedNutrition, recompute quickly
    this.selectedProducts.forEach((p) => {
      if (!p.calculatedNutrition && p.nutrition) {
        // Attempt approximate, fallback energy only
        p.calculatedNutrition = p.calculatedNutrition || p.nutrition;
      }
    });
  ...
  async searchProducts(query) {
    ...
    resultsContainer.innerHTML =
      '<div style="padding: 1rem; text-align: center;"><span class="spinner"></span> Searching...</div>';
    resultsContainer.classList.remove("hidden");

    try {
      const results = await this.openFoodFactsService.searchProducts(
        query,
        1,
        10,
      );
      ...
      resultsContainer.innerHTML =
        '<div style="padding: 1rem; color: #dc3545;">Search failed. Please try again.</div>';
    }
  ...
  addProductToMeal(product, quantity) {
    const productWithQuantity = {
      ...
      quantity,
      calculatedNutrition:
        this.openFoodFactsService.calculateNutritionForQuantity(
          product,
          quantity,
        ),
    };
  ...
  async saveMeal() {
    ...
    try {
      const meal = {
        id: this.edit && this.meal ? this.meal.id : undefined,
        name: mealName,
      ...
      if (this.edit && this.meal.id) {
        await this.storageService.updateMeal(meal);
        this.showAlert("Meal updated successfully!", "success");
      } else {
      ...
      setTimeout(() => {
        window.location.hash = "/meal-history";
      }, 1500);
    } catch (error) {
  ...
  openBarcodeScanner() {
    ...
    if (barcode) {
      this.openFoodFactsService
        ...
        .catch((error) =>
          this.showAlert("Product not found: " + error.message, "error"),
        );
    }
  }
