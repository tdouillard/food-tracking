...
export class StatsPage {
    ...
    container.innerHTML = `
      <div class="container">
        ...
        <div id="stats-content" class="hidden">
          <div class="card">
            <h3>Summary</h3>
            <div id="summary-stats" class="grid" style="grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 1rem;">
            </div>
          </div>
          ...
          <div class="grid grid-2">
            <div class="card">
              <h3>Daily Nutrition Trend</h3>
              <div class="chart-container">
                <canvas id="nutrition-trend-chart"></canvas>
              </div>
            </div>
            ...
            <div class="card">
              ...
              <div class="chart-container">
                <canvas id="meal-distribution-chart"></canvas>
              </div>
            </div>
          </div>
  ...
  attachEventListeners(container) {
    container.querySelector('#time-filter').addEventListener('change', async (e) => {
  ...
  async loadStats() {
    ...
    try {
      ...
      const meals = await this.storageService.getMeals(startDate.toISOString(), endDate.toISOString());
  ...
  renderCaloriesChart(meals) {
    ...
    const data = dateRange.map(date => {
      ...
      return dayMeals.reduce((total, meal) => total + (meal.totalNutrition?.energy || 0), 0);
    });
  ...
  renderNutritionTrendChart(meals) {
    ...
    const ctx = document.getElementById('nutrition-trend-chart').getContext('2d');
  ...
  renderMealDistributionChart(meals) {
    ...
    const ctx = document.getElementById('meal-distribution-chart').getContext('2d');
  ...
  calculateAverageCaloriesPerDay(meals) {
    ...
    const totalCalories = Object.values(dailyCalories).reduce((total, dayMeals) => {
      const dayCalories = dayMeals.reduce((dayTotal, meal) => 
        dayTotal + (meal.totalNutrition?.energy || 0), 0);
  ...
  async exportDataAsCSV() {
    try {
      ...
      this.downloadFile(csvContent, `food-tracking-${this.currentFilter}.csv`, 'text/csv');
    } catch (error) {
  ...
  async exportDataAsJSON() {
    try {
      ...
      this.downloadFile(jsonContent, `food-tracking-${format(new Date(), 'yyyy-MM-dd')}.json`, 'application/json');
    } catch (error) {
      alert('Export failed: ' + error.message);
    }
  }
  ...
  convertMealsToCSV(meals) {
    const headers = [
      'Date', 'Time', 'Meal Name', 'Product Name', 'Quantity', 
      'Calories', 'Proteins', 'Carbohydrates', 'Fat', 'Sugars', 'Fiber'
    ];
    ...
    meals.forEach(meal => {
      ...
      if (meal.products && meal.products.length > 0) {
        meal.products.forEach(product => {
          ...
          rows.push([
            date, time, meal.name, product.name, product.quantity,
