...
export class SettingsPage {
      ...
      <div class="container">
        ...
        <div class="card">
          ...
          <div class="form-group">
            ...
            <select class="form-select" id="storage-type">
              <option value="local">Local Storage (IndexedDB)</option>
              <option value="remote">Remote Storage (SQL Database)</option>
            </select>
            <small style="color: #666; margin-top: 0.5rem; display: block;">
              Local storage keeps your data on this device. Remote storage allows sync across devices but requires authentication.
            </small>
          </div>
          ...
          <div id="remote-settings" class="hidden">
            <div class="form-group">
              ...
              <input type="url" class="form-input" id="api-url" placeholder="https://your-api-server.com/api">
            </div>

            <div class="form-group">
              <label class="form-label">Authentication</label>
              <div id="auth-status" class="mb-2"></div>
              <div id="auth-form">
                <input type="email" class="form-input" id="email" placeholder="Email" style="margin-bottom: 0.5rem;">
                <input type="password" class="form-input" id="password" placeholder="Password" style="margin-bottom: 0.5rem;">
          ...
          <div class="form-group mt-2">
            <button class="btn btn-primary" id="save-storage-settings">Save Storage Settings</button>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div class="form-group">
            ...
            <small style="color: #666; margin-top: 0.5rem; display: block;">
              Import data from a JSON export or CSV file.
            </small>
          </div>

          <div class="form-group">
            <h4>Export Data</h4>
            <button class="btn btn-secondary" id="export-all-json">Export All Data (JSON)</button>
            <small style="color: #666; margin-top: 0.5rem; display: block;">
              Export all your meals and data as JSON file.
            </small>
          </div>

          <div class="form-group">
            <h4>Clear All Data</h4>
            <button class="btn btn-danger" id="clear-all-data">Clear All Local Data</button>
            <small style="color: #666; margin-top: 0.5rem; display: block;">
              ⚠️ This will permanently delete all locally stored meals and cannot be undone.
            </small>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <p><strong>Storage Type:</strong> <span id="current-storage-type"></span></p>
          <p><strong>Total Meals:</strong> <span id="total-meals">Loading...</span></p>
          <p><strong>Data Size:</strong> <span id="data-size">Calculating...</span></p>
          
          <div class="mt-2">
            <h4>Privacy & Data</h4>
            <p style="font-size: 0.9rem; color: #666; line-height: 1.5;">
              This app respects your privacy. When using local storage, your data never leaves your device. 
              When using remote storage, data is transmitted securely and stored according to your server's privacy policy.
            </p>
          </div>
        </div>
  ...
  async loadCurrentSettings() {
    ...
    if (storageType === 'remote') {
      ...
      document.getElementById('api-url').value = this.storageService.apiUrl || '';
  ...
  async updateAuthStatus() {
    ...
    if (this.storageService.isAuthenticated() && this.storageService.storageType === 'remote') {
      authStatus.innerHTML = '<div class="alert alert-success">✅ Authenticated and connected to remote storage</div>';
      authForm.classList.add('hidden');
      logoutBtn.classList.remove('hidden');
    } else {
      authStatus.innerHTML = '<div class="alert alert-warning">⚠️ Not authenticated. Please login to use remote storage.</div>';
  ...
  attachEventListeners(container) {
    ...
    container.querySelector('#save-storage-settings').addEventListener('click', async () => {
    ...
    container.querySelector('#import-data').addEventListener('click', async () => {
      await this.handleImportData();
    });

    container.querySelector('#export-all-json').addEventListener('click', async () => {
  ...
  async handleRegister() {
    ...
    try {
      ...
      this.showAlert('Registration functionality would connect to your API server. For now, use login with existing credentials.', 'warning');
    } catch (error) {
  ...
  async handleImportData() {
    ...
    try {
      ...
      this.showAlert(`Successfully imported ${importedCount} meals!`, 'success');
  ...
  async handleClearAllData() {
    const confirmation = prompt('This will delete ALL your local data. Type "DELETE" to confirm:');
  ...
  parseCSVData(csvContent) {
    ...
    Object.values(mealGroups).forEach(meal => {
      meal.totalNutrition = meal.products.reduce((total, product) => ({
        energy: total.energy + (product.calculatedNutrition.energy || 0),
        proteins: total.proteins + (product.calculatedNutrition.proteins || 0),
        carbohydrates: total.carbohydrates + (product.calculatedNutrition.carbohydrates || 0),
        ...
        energy: 0, proteins: 0, carbohydrates: 0, fat: 0, sugars: 0, fiber: 0
      });
